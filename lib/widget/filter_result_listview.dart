...
class FilterResultListView extends StatefulWidget {
  ...
  const FilterResultListView({this.state});

  @override
  _FilterResultListViewState createState() => _FilterResultListViewState(state);
}

class _FilterResultListViewState extends State<FilterResultListView> {
  String state;

  _FilterResultListViewState(this.state);
    ...
    return StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (allPosts.length == 0) {
            return Center(
              child:Text(Localization.instance.tr("ThereisnoPosts"),),
            );
          }
          return Container(
            child: ListView.builder(
              ...
              itemBuilder: (ctx, i) => ChangeNotifierProvider.value(
                value: allPosts[i],
                child: FilterResultListViewDesign(
                ),
              ),
            ),
          );
